...
/**
 * Test for {@link SumOfLogs} using {@code long} values.
 */
final class LongSumOfLogsTest extends BaseLongStatisticTest<DoubleAsLongStatistic> {

    @Override
    protected DoubleAsLongStatistic create() {
        return DoubleAsLongStatistic.from(SumOfLogs.create());
    }

    @Override
    protected DoubleAsLongStatistic create(long... values) {
        return DoubleAsLongStatistic.from(SumOfLogs.of(values));
    }
    ...
    protected Stream<StatisticTestData> streamTestData() {
        ...
        builder.accept(addReference(Double.NEGATIVE_INFINITY, DoubleTolerances.equals(), 0, 1, 2, 3, 4, Long.MAX_VALUE));
        // At least 1 negative; all other finite positive
        builder.accept(addReference(Double.NaN, DoubleTolerances.equals(), -1, 1, 2, 3, 4));
        // Extreme range
        builder.accept(addCase(Long.MAX_VALUE, Long.MAX_VALUE / 2, Long.MAX_VALUE / 4, 42));
