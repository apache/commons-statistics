...
/**
 * Test for {@link LongMin}.
 */
final class LongMinTest extends BaseLongStatisticTest<LongMin> {
    ...
    @Override
    protected LongMin create() {
        return LongMin.create();
    }

    @Override
    protected LongMin create(long... values) {
        return LongMin.of(values);
    }
    ...
    protected StatisticResult getEmptyValue() {
        return createStatisticResult(Long.MAX_VALUE);
    }

    @Override
    protected StatisticResult getExpectedValue(long[] values) {
        // Use the JDK as a reference implementation
        final long x = Arrays.stream(values).min().orElse(Long.MAX_VALUE);
